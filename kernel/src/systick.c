...
void systick_init(uint32_t frequency) {
    ...
    systick->LOAD = (BASE_FREQ/frequency) - 1;      // (16M cycles/s) / (freq ms/s) = 16M/freq cycles/ms
    systick->CTRL |= CLKSOURCE;             // Selects processor clock
    systick->CTRL |= TIMER_EN;              // Enable timer
    systick->CTRL |= TICK_EN;               // Enable systick interrupt
